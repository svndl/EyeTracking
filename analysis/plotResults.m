function plotResults(res, trackingType, plotType)
...
% deal with two possible data types
    ...
    cali_LE_mean = res.calibrationAvgLeftError;
    cali_RE_mean = res.calibrationAvgRightError;
    dyn = {'ramp'};
    dirs = {'left' 'right'};
    conds = {'SingleDot'};
    close all;
    ...
    for d = 1:length(dyn);
        ...
        for c = 1:length(conds)
            ...
            suptitle([res.subj ' ' dyn{d} ' ' plotType ' ( L err ' num2str(cali_LE_mean) ' R err ' num2str(cali_RE_mean) ' )']);
            ...
            for r = 1:length(dirs);
                ...
                % combination
                inds = strcmp(res.trials.condition, conds{c}) & ...
                        ...
                        case 'monocular'
                           if isempty(inds)
                                [ax,h1,h2] = plotyy(1:length(res.trials.predictionLE{inds(1)}),res.trials.predictionLE{inds(1)},...
                                    1:length(res.trials.predictionRE{inds(1)}),res.trials.predictionRE{inds(1)});hold on;
