function eyelink_RunCalibration (elStructure, key = 1)
...
% Emily renamed and cleaned up this PTB code
	...
	Eyelink( 'StartSetup' );                                    
	Eyelink( 'WaitForModeReady', elStructure.waitformodereadytime );     
	EyelinkClearCalDisplay(elStructure);                                


	key = 1;
	while key ~= 0
		key = EyelinkGetKey(elStructure);                              
	end


	if nargin == 2                                              
	
		if (elStructure.allowlocalcontrol)
		
			switch lower(sendkey)
				case{ 'c', 'v', 'd', elStructure.ENTER_KEY}
					forcedkey = double(sendkey(1,1));
					Eyelink('SendKeyButton', forcedkey, 0, elStructure.KB_PRESS );
				otherwise
			end
		end
	end

	tstartCalibration  = GetSecs;
	stopCalibration = 0;

	while (~stopCalibration) && bitand(Eyelink( 'CurrentMode'), elStructure.IN_SETUP_MODE)
		...
		if bitand(i, elStructure.IN_TARGET_MODE)			
...
% 		
		...
		switch key	
			...
			otherwise
			% Echo to tracker for remote control				
				if el.allowlocalcontrol==1
