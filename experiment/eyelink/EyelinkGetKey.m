function [key, el] = EyelinkGetKey(el)
	...
	%gp = Gamepad('GetButton', 1, 5);
	%resp1_1 = Gamepad('GetButton', 1, 14);
...
% 	if gp1+gp2+gp3+gp4+gp5 > 0
% 		
% 		keyCodes(el.escape)=1;
% 		keyIsDown = 1;
% 		WaitSecs(0.1);
% 	end
	...
	% convert to a character, potential problem: KbName does not translate all keys to actual ascii characters.	
	key = KbName(key(1));
	
	% some keyNames have two characters (e.g. 8*);	
	key = double(key(1)); 
	
	% only return  printable chars (0x20..0x7F)	
	if key >=hex2dec('20') && key < hex2dec('7F') 
		modifyKey;
		return;
	end
	% return code for untranslatable key
	key = el.JUNK_KEY;
	return;
end
