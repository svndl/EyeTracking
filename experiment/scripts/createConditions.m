function dataCombs = createConditions(data)

	
	%stimset all combvec
	columnLabels = {'dynamics', 'directions', 'isPeriodic', 'stimRadDeg', ...
		'dispArcmin', 'rampSpeedDegSec', 'dotSizeDeg', ...
		'dotDensity', 'cycleSec', 'trialRepeats'};
	cartesianTrialProd = allcomb(data.dynamics, data.directions, num2cell(data.isPeriodic), ...
