function timing = drawTrial(trialNum, dotsL, colorL, dotsR, colorR, stm, scr)
% draw dynamic part of trial simultaneously record
    ...
    timing = drawDots(dotsL, colorL, dotsR, colorR, stm, scr);
 	% stop recording
...
function timing = drawDots(dotsL, colorL, dotsR, colorR, stm, scr)
    ...
    while(idxUpdate <= size(dotsLE.x, 1))
        ...
        for r = 1:stm.dotRepeats
			...
			lDots = [dotsL.x(idxUpdate, :); dotsL.y(idxUpdate, :)];
			rDots = [dotsR.x(idxUpdate, :); dotsR.y(idxUpdate, :)];
