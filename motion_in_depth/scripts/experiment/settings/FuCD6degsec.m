...
dat.rampSpeedDegSec = 3;      % ramp speed in degrees per second
dat.dotSizeDeg      = 1;      % diameter of each dot
dat.dotDensity      = 0.5;      % dots per degree2

% timing
dat.preludeSec      = 0.25;      % delay before motion onset
dat.cycleSec        = 0.5;      % duration of stimulus after prelude

% conditions
dat.conditions      = {'CDOT','FullCue'};      % dot conditions, IOVD, CDOT, etc
dat.cond_repeats    = 20;      % number of repeats per condition
dat.dynamics        = {'ramp'};      % steps, ramps, etc
dat.directions      = {'away'};      % initial motion direction
